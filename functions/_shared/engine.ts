...
async function execute(step:Step, context:Record<string,unknown>, run:Run) {
 if(step.type==='llm_call') { const key = process.env.GEMINI_API_KEY || process.env.GEMINI_KEY || process.env.NEXT_PUBLIC_GEMINI_API_KEY || String(step.config.api_key || step.config.apiKey || ''); if(!key) return {stub:true,text:'LLM stub: configure GEMINI_API_KEY in Nhost Secrets or Step Config for a live Gemini response'}; const prompt=String(step.config.prompt||'Summarize this context: {{context}}').replace('{{context}}',JSON.stringify(context)); const r=await fetch(`https://generativelanguage.googleapis.com/v1beta/models/gemini-2.0-flash:generateContent?key=${key}`,{method:'POST',headers:{'content-type':'application/json'},body:JSON.stringify({contents:[{parts:[{text:prompt}]}]})}); if(!r.ok) throw new Error(`Gemini ${r.status}`); const d=await r.json() as any; return {text:d.candidates?.[0]?.content?.parts?.[0]?.text||''} }
